    ...
    async syncWithSupabaseIfAvailable() {
        try {
            ...
            if (userError || !userData || !userData.user) {
                console.warn('[LeaveStatus] Supabase 사용자 정보를 가져올 수 없어 로컬 데이터만 사용합니다.', userError);
                return;
            }
            ...
            if (grantError) {
                console.error('[LeaveStatus] Supabase welfare_leave_grants 로드 오류:', grantError);
            } else if (Array.isArray(grantRows)) {
                ...
                this.dataManager.welfareLeaveGrants = normalizedGrants;
                console.log('[LeaveStatus] ✅ Supabase 복지휴가 지급 이력 동기화 완료:', normalizedGrants.length, '건');
            }
        } catch (err) {
            console.error('[LeaveStatus] Supabase 동기화 중 예외:', err);
        }
    }
